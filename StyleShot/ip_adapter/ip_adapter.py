...
class StyleShot(torch.nn.Module):
    ...
    def generate(
        ...
        if encoder_svd:
            ...
            # exponential
            raw_style_ip_tokens = style_ip_tokens
            
            style_ip_tokens = embedding_svd_topk_exponential(
                raw_style_ip_tokens.float(),
                top_k=0, alpha=0.05, beta=1,
            )
            
            if encoder_cfg:
                print(f"[*****] specific negative style embedding")
                uncond_style_ip_tokens = embedding_svd_topk_exponential(
                    raw_style_ip_tokens.float(),
